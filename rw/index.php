...
$rt_ids_string = !empty($rt_ids_in_rw) ? implode(',', $rt_ids_in_rw) : 'NULL';


$stmt_total = $conn->prepare("SELECT COUNT(id_pengaduan) as total FROM pengaduan WHERE tujuan_id_rw = ? OR tujuan_id_rt IN ($rt_ids_string)");
$stmt_total->bind_param("i", $id_rw_session);
$stmt_total->execute();
$total_pengaduan = $stmt_total->get_result()->fetch_assoc()['total'] ?? 0;
$stmt_total->close();

$stmt_proses = $conn->prepare("SELECT COUNT(id_pengaduan) as total FROM pengaduan WHERE (tujuan_id_rw = ? OR tujuan_id_rt IN ($rt_ids_string)) AND status = 'Diproses'");
$stmt_proses->bind_param("i", $id_rw_session);
$stmt_proses->execute();
$pengaduan_diproses = $stmt_proses->get_result()->fetch_assoc()['total'] ?? 0;
$stmt_proses->close();

$stmt_selesai = $conn->prepare("SELECT COUNT(id_pengaduan) as total FROM pengaduan WHERE (tujuan_id_rw = ? OR tujuan_id_rt IN ($rt_ids_string)) AND status = 'Selesai'");
$stmt_selesai->bind_param("i", $id_rw_session);
...
$stmt_kategori->bind_param("i", $id_rw_session);
...
$stmt_terbaru->bind_param("i", $id_rw_session);
...
<head>
    ...
    <title>Dashboard - Panel RW</title>
