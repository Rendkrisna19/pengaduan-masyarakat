...
if (!isset($_SESSION['user_id']) || $_SESSION['user_role'] !== 'RT' || empty($_SESSION['user_rt_id'])) {
    die('Akses ditolak.');
}
...
// Logika query yang sama persis dengan di halaman laporan RT
$params = [];
$types = '';
$query_laporan = "SELECT p.id_pengaduan, p.tanggal_lapor, u.nama_lengkap as nama_pelapor, k.nama_kategori, p.deskripsi, rt.nomor_rt, rw.nomor_rw, p.status 
                  ...
                  WHERE p.tujuan_id_rt = ? AND p.tanggal_lapor BETWEEN ? AND ?";
...
if ($format == 'csv') {
    $filename = "laporan_rt_" . date('Y-m-d') . ".csv";
    ...
    $html .= '<h1>Laporan Pengaduan Wilayah RT</h1>';
    ...
    $dompdf->render();
    $filename = "laporan_rt_" . date('Y-m-d') . ".pdf";
