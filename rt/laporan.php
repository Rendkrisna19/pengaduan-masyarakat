...
if (!isset($_SESSION['user_id']) || $_SESSION['user_role'] !== 'RT') { header("Location: ../auth/login.php"); exit(); }
if (empty($_SESSION['user_rt_id'])) { die("Error: Akun RT Anda tidak terhubung dengan data wilayah."); }
...
$params = []; $types = '';
$query_laporan = "SELECT p.id_pengaduan, p.tanggal_lapor, p.deskripsi, p.status, u.nama_lengkap as nama_pelapor, k.nama_kategori, rt.nomor_rt, rw.nomor_rw FROM pengaduan p JOIN users u ON p.id_user_pelapor = u.id_user JOIN kategori_pengaduan k ON p.id_kategori = k.id_kategori JOIN rt ON p.id_rt_lokasi = rt.id_rt JOIN rw ON rt.id_rw = rw.id_rw WHERE p.tujuan_id_rt = ? AND p.tanggal_lapor BETWEEN ? AND ?";
$params[] = $id_rt_session; $params[] = $tanggal_mulai; $params[] = $tanggal_akhir;
$types .= 'iss';
if ($status_filter != 'Semua' && $status_filter != '') { $query_laporan .= " AND p.status = ?"; $params[] = $status_filter; $types .= 's'; }
$query_laporan .= " ORDER BY p.tanggal_lapor DESC";
$stmt = $conn->prepare($query_laporan); $stmt->bind_param($types, ...$params); $stmt->execute();
$laporan_list = $stmt->get_result()->fetch_all(MYSQLI_ASSOC); $stmt->close();
...
<head>
    ...
    <title>Laporan Wilayah - Panel RT</title>
            ...
            <div class="main-content">
                <div class="card shadow mb-4">
                    <div class="card-header py-3">
                        <h6 class="m-0 font-weight-bold" style="color: var(--primary-color);">Filter Laporan Wilayah RT
                            Anda</h6>
                    </div>
                    <div class="card-body">
                        <form action="laporan.php" method="GET" id="filter-form">
                            <div class="row g-3 align-items-end">
                                ...
                                <div class="col-md-1"><button type="submit" name="filter" value="true"
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (cetakBtn) {
            cetakBtn.addEventListener('click', function() {
                ...
                printWindow.document.write('<html><head><title>Cetak Laporan RT</title>');
                ...
                printWindow.document.write('<h3>Laporan Pengaduan Wilayah RT</h3><hr>');
